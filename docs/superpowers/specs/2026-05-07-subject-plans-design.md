...
- 학교명·날짜를 인쇄 헤더로 추가 (`schoolName ?? '학교'` + `new Date().toLocaleDateString()`)
...
| 전담 시수 합계 > 주당 총합 | 위젯 빨간색, `(초과 -N시간)` 표시. 적용은 가능하되 confirm에 경고 추가 검토(MVP 단계에선 단순 경고 표시만) |
...
- `과목 시수 합계 > 주당 총합` 케이스에서 적용을 막을지 단순 경고만 띄울지 — MVP에서는 경고만, 사용자 피드백 후 결정
- 인쇄 시 학교명/날짜를 표 안 어디에 넣을지의 정확한 레이아웃 — 구현 단계에서 시각 확인하며 조정
