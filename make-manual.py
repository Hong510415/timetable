...
story.append(Paragraph("문서 버전: 2026-07 (학기제 반영)", caption_style))
...
story.append(kv_table([
    ...
    ("학기별 배정 사용", "‘교사 설정’ 탭 하단에서 켜면 1·2학기 전담 배정을 다르게 구성할 수 있습니다. 끄면 기존(연간 기준)과 동일하게 동작합니다."),
]))
...
story.append(Paragraph("1년/1학기/2학기 구분 (학기제 ON)", h2_style))
story.append(bullet([
    "학교 설정에서 ‘학기별 배정 사용’을 켜면 과목마다 1년·1학기·2학기를 선택하는 칸이 나타납니다.",
    "1학기 또는 2학기만 하는 과목은 담임·전담 시수 요약에 0.5배로 계산됩니다 (예: 1학기 주 3시간 → 연간 환산 1.5시간).",
    "같은 과목의 1·2학기 수업은 전담 배정에서 한 교사에게 묶이고, 시간표에서 같은 요일·교시에 짝으로 배치됩니다.",
]))
...
story.append(bullet([
    ...
    "교사 고정: 교사명 옆 ‘고정’을 체크하면 그 교사의 배정은 유지되고, 자동 배정 실행 시 나머지 교사만 다시 분배됩니다(일부만 손대고 싶을 때 유용).",
]))

story.append(Paragraph("수동 편집", h2_style))
story.append(bullet([
    "자동 배정 후 표에서 행 단위로 수정 가능. ‘편집’에서 담당 반은 물론 학년(같은 과목이 있는 학년)도 변경할 수 있습니다.",
    "교사명(예: 34영어)은 표에서 직접 입력·수정할 수 있습니다.",
    "‘+ 과목 설정 추가’ 버튼으로 새 행 추가 (수동 배정).",
    "시수 부족·초과 등 균형 문제가 있으면 경고가 표시됩니다.",
    "※ 한 반의 한 과목을 두 교사가 시수를 나눠 맡는 경우(예: 통합 1시간씩)는 전담 시간표에서 칸을 직접 클릭해 교사별로 입력하면 됩니다.",
]))
...
story.append(Paragraph("학기 과목 배정 (학기제 ON)", h2_style))
story.append(bullet([
    "1·2학기로 구분한 과목은 일단 정상 배정한 뒤, 같은 과목을 한 교사에게 묶습니다.",
    "학기 과목은 시수 요약에 0.5배로 반영되며, 시수 편차가 크면 학기 구분이 없는 연간 일반과목만 조정해 균형을 맞춥니다 (주요 과목·학기 과목은 건드리지 않음).",
    "배정 표의 행 옆 배지로 1학기/2학기가 표시됩니다.",
]))
...
story.append(bullet([
    ...
    "표 아래에 부족 원인(요일 몰림·특별실 제약·과목 연속 제한 등)과 해결 방법 안내가 함께 표시됩니다.",
    "수동 편집으로 미배정 셀을 채우면 표에서 자동 제거됩니다 (실시간 동기화).",
    "해결: 교사 수·담당 조정 → 과목 연속·하루 최대 시수 완화 → 특별실 사용 불가 시간 축소 → 미배정 칸 직접 클릭해 수동 배치.",
]))
...
story.append(kv_table([
    ...
    ("학급별 보기", "학년·반 선택 (여러 반 동시 선택 가능, ‘전체 선택’·‘전체 해제’ 지원). 학급의 전담 수업만 표시됩니다."),
]))

story.append(Paragraph("1·2학기 구분 보기 (학기제 ON)", h2_style))
story.append(bullet([
    "학교 설정에서 ‘학기별 배정 사용’을 켜면 보기 버튼 옆에 1학기·2학기 전환 버튼이 나타납니다.",
    "선택한 학기의 수업만 표시됩니다 (연간 과목·외부강사는 항상 표시).",
    "교사별·학급별·특별실 엑셀도 현재 보는 학기 기준으로 나눠 다운로드됩니다 (파일명에 학기 표시).",
...
story.append(Paragraph("Q. 1·2학기를 다르게 편성하려면? (학기제)", h3_style))
story.append(bullet([
    "학교 설정 → ‘교사 설정’ 탭 하단의 ‘학기별 배정 사용’을 켭니다.",
    "전담 과목 설정에서 과목마다 1년/1학기/2학기를 지정합니다 (1·2학기 과목은 시수 0.5배 반영).",
    "같은 과목의 1·2학기는 한 교사에게 묶여 같은 요일·교시에 짝으로 배치됩니다.",
    "전담·특별실 시간표의 1학기/2학기 버튼으로 나눠 보고, 엑셀도 학기별로 내려받습니다.",
    "학기제를 끄면 모든 기능이 기존과 동일하게(연간 기준) 동작합니다.",
]))
